...
public interface ShellCallback {
   ...
   * </ul>
   *
   * The default shell callback interprets both values as directories and simply concatenates the two values to generate
   * the default directory.
   *
   * @param targetProject the target project
   * @param targetPackage the target package
   * @return the directory (must exist)
   * @throws ShellException if the project/package cannot be resolved into a directory on the file system. In this case,
   *         the generator will not save the file it is currently working on. The generator will add the exception
   ...
   * @param fileEncoding the file encoding for reading existing Java files. Can be null, in which case the platform
   ...
   *         saved and the existing file will remain undisturbed. The generator will add the exception message to the
   *         list of warnings automatically.
   */
  String mergeJavaFile(String newFileSource, String existingFileFullPath, String[] javadocTags, String fileEncoding)
      throws ShellException;

  /**
   * After all files are saved to the file system, this method is called once for each unique project that was affected
   * by the generation run. This method is useful if your IDE needs to be informed that file system objects have been
   * created or updated. If you are running outside of an IDE, your implementation need not do anything in this method.
   *
  ...
  /**
   * Return true if the callback supports Java merging, otherwise false. The <code>mergeJavaFile()</code> method will be
   * called only if this method returns <code>true</code>.
   *
